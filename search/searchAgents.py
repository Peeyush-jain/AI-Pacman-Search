    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if hitsWall == 0:
                ...
                st1[0] = nextState
                cornerList = st1[1:]
                if (st1[0] in self.corners and st1[0] not in cornerList):
                    st1.append(st1[0])
                    

                successors.append( ( tuple(st1) , action,1) )
...
def cornersHeuristic(state, problem):
    ...
    while (len(remCorners) > 0):
        min = manhattanDistance(current,remCorners[0])
        current1 = remCorners[0]
        for i in remCorners:
            d = manhattanDistance(current,i)
            if (d < min):
                min = d
                current1 = i

        total = total + min         
        remCorners.remove(current1)
        current = current1

    return total # Default to trivial solution

def manhattanDistance(current,corner):
...
def foodHeuristic(state, problem):
    ...
    maximum = 0 
    for cordinate in foodList:
        maximum = max(maximum,mazeDistance(position,cordinate,problem.startingGameState))
